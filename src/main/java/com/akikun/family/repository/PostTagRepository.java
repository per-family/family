...
public interface PostTagRepository extends BaseRepository<PostTag, Integer> {
     ...
     */
    @Query("select new com.akikun.family.model.projection.TagPostPostCountProjection("
        + "count(pt.postId), pt.tagId) from PostTag pt where pt.tagId in ?1 group by pt.tagId")
    @NonNull
     ...
     */
    @Query("select new com.akikun.family.model.projection.TagPostPostCountProjection("
        + "count(pt.postId), pt.tagId) from PostTag pt inner join Post p on p.id=pt.postId and"
