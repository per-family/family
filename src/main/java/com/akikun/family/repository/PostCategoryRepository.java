...
public interface PostCategoryRepository extends BaseRepository<PostCategory, Integer> {
     ...
     */
    @NonNull
    @Query("select new com.akikun.family.model.projection.CategoryIdPostStatusProjection"
        + "(pc.categoryId, pc.postId, p.status)"
        + " from PostCategory pc inner join Post p on p.id=pc.postId")
